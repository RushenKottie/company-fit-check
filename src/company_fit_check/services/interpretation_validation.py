"""Interpretation validation for the current workflow."""
...
from company_fit_check.models.state import Axis
...
def validate_interpretation(
    ...
    """Validate the interpretation and ask for clarifications when needed."""

    logger.info(
        "Validating interpretation axes=%s simplified_cv_chars=%s prompt_chars=%s",
        len(axes),
    ...
    if messages:
        logger.info("Interpretation validation needs clarification message_count=%s", len(messages))
        return False, " ".join(messages)

    if axis_issue_message:
        logger.info("Interpretation validation failed deterministic axis checks.")
        return False, axis_issue_message

    logger.info("Interpretation validation passed without clarification.")
    return True, None
        ...
        messages = [
            SystemMessage(
                content=(
                    ...
                    "extra axes. Apply this rule strictly: absolute or near-absolute "
                    "company-side facts such as location, size, domain, stage, role "
                    "family, and work mode belong to search criteria, not axes. Axes "
                    "are only the parts of the user's request that require "
                    "interpretation, investigation, assumptions, or probabilistic "
                    "judgment about fit. "
                    "Never ask the user to add axes for company-side search filters. "
                    "Broad but intentional axes such as English-working environment, "
                    "career-switch openness, stack and skills match, compensation fit, "
                    "or visa eligibility are valid if they are clear enough to score "
                    "using CV evidence and company or role data. "
