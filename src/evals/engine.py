...
from evals.models import CaseExecutionResult, EvalCase, EvalStubSpec
from evals.stubs import STUB_REGISTRY
from graph import nodes as graph_nodes
from graph.workflow import apply_clarification, create_initial_state, run_workflow
...
NODE_TARGETS = {
    "extract_and_mask_cv": graph_nodes.extract_and_mask_cv_node,
    "simplify_cv": graph_nodes.simplify_cv_node,
    "interpret_user_input": graph_nodes.interpret_user_input_node,
    "validate_pii_masking": graph_nodes.validate_pii_masking_node,
    "validate_user_input_interpretation": graph_nodes.validate_user_input_interpretation_node,
    "search_companies": graph_nodes.search_companies_node,
    "score_companies": graph_nodes.score_companies_node,
}
HELPER_TARGETS = {
    "score_payload_validator": graph_nodes.validate_company_score_payload,
...
def _execute_workflow_case(case: EvalCase, *, run_id: str) -> CaseExecutionResult:
    ...
    for clarification in case.clarifications:
        final_state = apply_clarification(final_state, clarification)
    ...
    return CaseExecutionResult(
        ...
        final_state_summary=graph_nodes.build_case_result_summary(final_state),
...
def _execute_node_case(case: EvalCase, *, run_id: str) -> CaseExecutionResult:
    node = NODE_TARGETS.get(case.entrypoint.target)
    if node is None:
...
def _execute_helper_case(case: EvalCase, *, run_id: str) -> CaseExecutionResult:
    ...
    return CaseExecutionResult(
        ...
        final_state_summary=graph_nodes.build_case_result_summary(state),
