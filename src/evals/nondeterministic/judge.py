"""LLM-as-judge helpers for non-deterministic evaluation runs."""
...
def get_default_nondeterministic_judge_config() -> NondeterministicJudgeConfig:
    """Return the default prompt and metric definitions for judge scoring."""

    return NondeterministicJudgeConfig(
        system_prompt=(
            ...
            "constraints were not ignored. When judging the final company list, act "
            "at least as carefully as the agent. Treat factual objections as a "
            "burden-of-proof problem: lower a score for company-list correctness only "
            "when the case material or verified current public web evidence available "
            "to you proves a concrete mismatch. Internal model knowledge, intuition, "
            "or stale memory is not ground truth. If you suspect a company is wrong, "
            "misclassified, outside the required region, inactive, or otherwise "
            "misaligned, verify that suspicion against public web evidence before "
            "using it as a penalty. If verification is unavailable, would require "
            "broad research across a large list, or remains ambiguous, trust the "
            "agent's candidate as plausible and do not punish it. Do not claim or "
            "imply web verification unless browsed sources or web-tool results are "
            "actually available in the judging context. Be "
        ...
        metrics=[
            JudgeMetricDefinition(
                ...
                rubric=(
                    ...
                    "prompt. Consider user tone, patience, and prior turns: with terse, "
                    "messy, suspicious, or irritated users, fewer focused questions can "
                    "be better than trying to resolve every ambiguity. Do not use a "
                    "broadly acceptable final CSV to excuse a missed "
            ...
            JudgeMetricDefinition(
                ...
                rubric=(
                    ...
                    "presented as provisional. Consider user tone, clarity, patience, "
                    "and prior turns: with terse, irritated, messy, or privacy-protective "
                    "users, concise progress with reasonable assumptions can be "
                    "appropriate. Punish assumptions mainly when they override explicit "
                    "criteria, invent important preferences, or materially mislead the "
                    "search. Do not let a decent final company list "
            ...
            JudgeMetricDefinition(
                ...
                rubric=(
                    ...
                    "possible companies. Judge hard criteria strictly, but treat axes "
                    "as subjective fit signals. The CSV only needs to be a plausible "
                    "starter list for user review, not an optimal human-curated search. "
                    "For factual objections to individual companies, use a prove-it "
                    "standard. Do not penalize a company because it seems questionable "
                    "from your internal memory. Penalize only when the generated CSV, "
                    "conversation, CV, or verified current public web evidence available "
                    "to you proves that the company violates an explicit user constraint "
                    "or that the rationale makes a materially false claim. If public "
                    "verification is unavailable, the list is too large to verify "
                    "company-by-company, the question would require broad research, or "
                    "the evidence is not obvious, treat the company as plausible and "
                    "give the agent the benefit of the doubt. Do not claim or imply "
                    "web verification unless browsed sources or web-tool results are "
                    "actually available in the judging context. "
                    "Weakly stated signals such as maybe, nice-to-have, especially, "
                    "or examples may guide search, but they are not negative "
                    "constraints. Over-indexing on one optional subtype such as drones, "
                    "defense, robotics, startups, or enterprise firms is at most a "
                    "quality weakness unless it violates an explicit diversity, balance, "
                    "or exclusion constraint, or displaces the core requested field. "
                    "Do not penalize merely generic, incomplete, or "
                    ...
                    "false justifications that materially break alignment. Score 2 only "
                    "when there is proven concrete misalignment: many companies do not "
                    "plausibly match the core requested field, an explicit hard "
                    "constraint is violated, or rationales make materially false "
                    "claims. Score 3 when the list satisfies explicit hard constraints "
                    "and most companies are plausibly relevant to the core request, even "
                    "if the list is generic, imbalanced, repetitive, over-indexed on a "
                    "nice-to-have, or weakly explained. Score 4 when recommendations and "
...
def judge_nondeterministic_case(
...
def _build_judge_prompt(request: NondeterministicJudgeRequest) -> str:
    ...
    return (
        ...
        "- Use the provided metric definitions, case material, and any verified current public web evidence actually available to you.\n"
        "- Do not treat internal model memory or unverified assumptions as factual ground truth.\n"
        "- For company-list factual objections, use a prove-it standard: prove a concrete mismatch before lowering a score.\n"
        "- If public verification is unavailable, too broad, too difficult, ambiguous, or not obvious, trust the agent's company candidate as plausible.\n"
        "- Do not claim or imply web verification unless browsed sources or web-tool results are actually available in the judging context.\n"
