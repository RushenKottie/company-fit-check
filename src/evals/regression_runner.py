"""Non-deterministic regression runner using the shared session entrypoint."""
...
from evals.judge import get_default_regression_judge_config, judge_regression_case
from evals.models import (
    RegressionJudgeOutcome,
...
class RegressionRunner:
    ...
    def run_cases(
        ...
        *,
        ...
        results: list[RegressionRunResult | None] = [None] * len(ordered_case_ids)
    ...
    def _judge_results(self, results: list[RegressionRunResult]) -> list[RegressionRunResult]:
        ...
        judge_config = get_default_regression_judge_config()
    ...
    def _judge_one_result(
        ...
        *,
        ...
        """Judge one regression case result and log metrics to the same MLflow run."""
        llm_called = False
        try:
            ...
            request = RegressionJudgeRequest(
            ...
            llm_called = True
            response = judge_regression_case(request)
...
def run_case(case_id: int) -> RegressionRunResult:
    """Run and judge one case with the default shared simulator."""

    return create_default_regression_runner().run_cases([case_id])[0]
...
def run_cases(
    ...
    *,
    ...
    return create_default_regression_runner().run_cases(
        case_ids,
...
def _build_regression_run_name(*, case_id: int, profession: str, run_id: str) -> str:
    """Return a compact MLflow display name for one regression case run."""
