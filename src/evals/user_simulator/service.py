...
@lru_cache(maxsize=1)
def load_cached_cases_by_id() -> Mapping[int, NondeterministicCase]:
    """Load the case JSON files once and reuse the ``case_id -> case`` map."""

    return build_nondeterministic_case_index()


def list_available_case_ids() -> list[int]:
    """Return the sorted non-deterministic case ids."""
...
class UserSimulator:
    ...
    def __init__(
        self,
        *,
        case_index: Mapping[int, NondeterministicCase] | None = None,
